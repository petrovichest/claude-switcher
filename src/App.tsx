...
function App() {
  ...
  const [isScheduledWarmupsExpanded, setIsScheduledWarmupsExpanded] = useState(false);
  ...
  const [processInfo, setProcessInfo] = useState<ClaudeProcessInfo | null>(null);
  ...
  const [otherAccountsSort, setOtherAccountsSort] = useState<
    "plan_asc" | "plan_desc" | "name_asc" | "name_desc"
  >("plan_asc");
  ...
  const checkProcesses = useCallback(async () => {
    try {
      const info = await invoke<ClaudeProcessInfo>("check_claude_processes");
      setProcessInfo(info);
    } catch (err) {
  ...
  const handleSwitch = async (accountId: string) => {
    try {
      ...
      const latestProcessInfo = await invoke<ClaudeProcessInfo>("check_claude_processes").catch(
        (err) => {
      ...
      const hasRunningClaude =
        !!latestProcessInfo &&
        (latestProcessInfo.count > 0 || latestProcessInfo.background_count > 0);

      let restartRunningClaude = false;
      if (hasRunningClaude) {
        restartRunningClaude = window.confirm(
          `Claude Code is running (${latestProcessInfo.count} foreground, ${latestProcessInfo.background_count} background). Do you want Claude Switcher to close and reopen it gracefully before switching accounts?`
        );

        if (!restartRunningClaude) {
          return;
        }
      }

      await switchAccount(accountId, restartRunningClaude);
  ...
  const handleExportFullFile = async () => {
    try {
      ...
      const selected = await save({
        ...
        defaultPath: "claude-switcher-full.cswf",
        filters: [
          {
            name: "Claude Switcher Full Backup",
            extensions: ["cswf"],
          },
        ],
      });
  ...
  const hasRunningProcesses =
    !!processInfo && (processInfo.count > 0 || processInfo.background_count > 0);
  ...
  const sortedOtherAccounts = useMemo(() => {
    return [...otherAccounts].sort((a, b) => {
      const planA = a.usage?.plan_type || a.plan_type || "";
      const planB = b.usage?.plan_type || b.plan_type || "";
      if (otherAccountsSort === "plan_asc" && planA !== planB) {
        return planA.localeCompare(planB);
      }
      if (otherAccountsSort === "plan_desc" && planA !== planB) {
        return planB.localeCompare(planA);
      }
      if (otherAccountsSort === "name_desc") {
        return b.name.localeCompare(a.name);
      }
      if (otherAccountsSort === "name_asc") {
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {processInfo && (
                    ...
                    >
                      ...
                      <span>
                        {hasRunningProcesses
                          ? `${processInfo.count} Claude running`
                          : "0 Claude running"}
                      </span>
                    </span>
                  )}
        ...
        {loading && accounts.length === 0 ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-500 mb-6">
              Add your first Claude account to get started
            </p>
            <button
          ...
          <div className="space-y-8">
            <section className="rounded-2xl border border-gray-200 bg-white px-4 py-3 shadow-sm">
              <button
                type="button"
                onClick={() => setIsScheduledWarmupsExpanded((prev) => !prev)}
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    ...
                    {appSettings?.scheduled_warmup?.enabled && nextScheduledRunLabel && (
                      <span className="rounded-full bg-gray-100 px-2 py-0.5 text-[11px] font-medium text-gray-600">
                        Next: {nextScheduledRunLabel}
                      </span>
                    )}
                  </div>
                </div>
                <span className="shrink-0 rounded-full bg-gray-100 px-2.5 py-1 text-[11px] font-medium text-gray-600">
                  {isScheduledWarmupsExpanded ? "Hide" : "Show"}
                </span>
              </button>

              {isScheduledWarmupsExpanded && (
                <div className="mt-3 flex flex-col gap-3 border-t border-gray-100 pt-3 md:flex-row md:items-center md:justify-between">
                  <p className="text-sm text-gray-500">
                    {appSettings?.scheduled_warmup?.enabled
                      ? "Adjust the daily time and which accounts are included."
                      : "Configure a local time and choose which accounts should be warmed automatically."}
                  </p>
                  <button
                    onClick={() => setIsScheduledWarmupsModalOpen(true)}
            ...
            {activeAccount && (
              <section>
                ...
                <div className="grid justify-start [grid-template-columns:repeat(auto-fit,minmax(280px,320px))]">
                  <AccountCard
            ...
            {otherAccounts.length > 0 && (
              <section>
                ...
                <div className="grid justify-start gap-4 [grid-template-columns:repeat(auto-fit,minmax(280px,320px))]">
